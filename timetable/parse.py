...
def parse_all():
    ...
    for url, html in raw:
        try:
            ...
            raise
    results.to_sql(
        "stage", sql_engine, schema="ODS_TIMETABLE", index=False, if_exists="replace"
    )
    return len(results)
...
@task(
    inlets=[
        Dataset("STG_TIMETABLE.raw_html"),
        Dataset("ODS_TIMETABLE.stage"),
        Dataset("ODS_TIMETABLE.hist"),
    ],
