...
def download_pages_to_db():
    ...
    sql_engine.execute(
        'ALTER TABLE IF EXISTS "STG_TIMETABLE".raw_html RENAME TO raw_html_old;'
    )
    logging.info("Table raw_html renamed to raw_html_old")

    sql_engine.execute(
        'CREATE TABLE "STG_TIMETABLE".raw_html (url VARCHAR(256), raw_html TEXT);'
    )
    logging.info("New raw_html table created")

    data.to_sql(
        "raw_html", sql_engine, schema="STG_TIMETABLE", if_exists="append", index=False
    )
