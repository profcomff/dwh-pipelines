...
with DAG(
    ...
    PostgresOperator(
        ...
        sql=dedent(r"""INSERT INTO ODS.User.union_member (fio, card_number, card_date, card_status) 
SELECT 
  a.full_name, 
  a.student_id_number, 
  b.card_date, 
...
FROM 
  ODS.user_info AS a 
  LEFT JOIN STG.Union_member.Union_member AS b ON (a.full_name = CONCAT(b.first_name, ' ', b.middle_name, ' ', b.last_name)) 
  AND (a.student_id_number = b.card_number);
"""
        ), #проверка на правильный джойн
    PostgresOperator(
        postgres_conn_id="postgres_dwh",
        sql=dedent(r"""
        WITH 
